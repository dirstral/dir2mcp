...
# Exits non-zero when any pair failed, after trying every pair.
set -uo pipefail
R=${1:?results dir}
C=${2:?corpus dir}
cd "$(dirname "$0")/../../.." || exit 1
...
pair() {
  echo "=== $1  A=$2  B=$3"
  # sed reads the whole stream, so python's exit status is the pipeline's
  # (head would close the pipe early and mask a failure).
  if ! python3 -m tools.rfe_rig --results "$R" agreement --recording "$C/$1" --a "$2" --b "$3" | sed -n '1,12p'; then
    echo "!!! FAILED $1 $2 vs $3"
    failed=$((failed + 1))
  fi
}
...
if [ "$failed" -ne 0 ]; then
  echo "AGREEMENT: $failed pair(s) failed"
  exit 1
fi
echo "AGREEMENT: all pairs written"
