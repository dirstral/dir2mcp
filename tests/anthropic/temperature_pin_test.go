...
func (r *tempRecorder) server(t *testing.T) *httptest.Server {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		case refuse:
			w.WriteHeader(http.StatusBadRequest)
			_, _ = w.Write([]byte(`{"type":"error","error":{"type":"invalid_request_error","message":"Unsupported parameter: temperature is not supported when thinking is enabled."}}`))
			return
		}
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"content":[{"type":"text","text":"grounded answer"}]}`))
	}))
	t.Cleanup(srv.Close)
	return srv
}
