...
// the name-hint augmentation rewrites the translation prompt, so switching it on
// must MISS the cache rather than return the un-hinted translation produced
// before. This was observed in practice — a re-index after enabling hints
// returned byte-identical output because every line was served from cache.
//
// It also locks the converse, which is why the key folds the hint GATE and not
// the raw source language: hints only reach the prompt for a Russian source and
// an English target, so with hints enabled but a source that cannot take them
// the prompt is byte-identical to the un-hinted one and the existing cache entry
// must still hit. Keying on the raw source language would throw that entry away
...
func TestTranslateCacheKey_NameHintModeMisses(t *testing.T) {
	...
	// Hints enabled but unreachable: non-Russian source, and non-English target.
	// Both leave the prompt untouched, so both must keep hitting the off key.
