...
// TestMediaTranslateNameHints_DefaultsOffAndRoundTrips pins that proper-noun spelling
// hints are OFF by default (so existing prompts are unchanged) and that the key binds
// from nested YAML and survives a save/load round-trip.
func TestMediaTranslateNameHints_DefaultsOffAndRoundTrips(t *testing.T) {
	cfg := config.Default()
	if cfg.MediaTranslateNameHints {
	...
	loaded, err := config.LoadFile(path)
	if err != nil {
		t.Fatalf("LoadFile(nested name_hints): %v", err)
	}
	if !loaded.MediaTranslateNameHints {
		t.Fatalf("nested media.translate.name_hints not applied")
	}
