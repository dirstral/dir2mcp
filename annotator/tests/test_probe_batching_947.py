...
def _long_run(monkeypatch, tmp_path, n):
    ...
    return lambda ps: [(text, 0.9) for _ in ps]


def test_probe_calls_never_exceed_batch_size(monkeypatch, tmp_path):
    captioner = _long_run(monkeypatch, tmp_path, 20)
    seen = []
    ...
    assert max(seen) <= 8, f"a probe call exceeded batch_size: {seen}"
    assert sum(seen) == 20, f"every frame of the run must be probed exactly once: {seen}"
    assert seen == [8, 8, 4]
