...
func TestProfileSettingsYAML_LoadsBack(t *testing.T) {
	...
	after := before
	setupwizard.ApplyCorpusProfile(&after, setupwizard.ProfileLegal)
	lines := profileSettingsYAML(before, after)
	...
	if strings.TrimSpace(got.RAGSystemPrompt) != strings.TrimSpace(after.RAGSystemPrompt) {
		t.Errorf("system_prompt did not load back:\ngot  %q\nwant %q", got.RAGSystemPrompt, after.RAGSystemPrompt)
	}
}

// TestProfileSettingsYAML_NothingToAdd pins that the general profile on a
// default config prints nothing.
func TestProfileSettingsYAML_NothingToAdd(t *testing.T) {
	before := config.Default()
	after := before
	setupwizard.ApplyCorpusProfile(&after, setupwizard.ProfileKeep)
	if lines := profileSettingsYAML(before, after); lines != nil {
